...
def process_video(video_path, output_path, background, mask, max_frames=400, max_contour_area=3000, max_movement=50, frame_window=30):
    try:
        ...
        # Initialize a list to store the fly positions and largest contours
        fly_positions = []
        largest_contours = []
        area = np.array([])
        frame_count = 0
        ...
        while True:
            ...
            _, thresholded = cv2.threshold(background_diff, 150, 255, cv2.THRESH_BINARY)
            ...
            if contours:
                ...
                if valid_contours:
                    ...
                    # Calculate the average area of the largest contours
                    avg_largest_contour_area = np.mean([cv2.contourArea(c) for c in largest_contours])



                    # Fit an ellipse to the largest contour if it has at least 5 points
    # Fit an ellipse to the largest contour if it has at least 5 points
            if len(largest_contour) >= 5:
                ellipse = cv2.fitEllipse(largest_contour)

                # Check if the ellipse size is valid
                if ellipse[1][0] > 0 and ellipse[1][1] > 0:
                    ...
                    if cv2.contourArea(largest_contour) < avg_largest_contour_area / 1.2:
                                ...
                                index_to_delete = np.where(area == cv2.contourArea(largest_contour))[0]
                                if index_to_delete.size > 0:
                                    area = np.delete(area, index_to_delete[0])
        ...
        # Resize the video frame
            ...
            ax.plot(area, label="Contour Area")
        ...
        # Render the plot to a canvas
            ...
            combined_frame = np.hstack((frame, plot_image))  # Use 'frame' instead of 'masked_frame'
            ...
            # Clear the plot and update it with new data
            ...
            # Render the plot to a canvas again
            canvas.draw()
            plot_image = np.frombuffer(canvas.tostring_rgb(), dtype=np.uint8)
            plot_image = plot_image.reshape(canvas.get_width_height()[::-1] + (3,))
        ...
        print('error due to no fly')

    return contour_found_in_video, contour_below_threshold_count

# Set the path to the folder containing the videos
input_folder = '/Users/tairan/Downloads/rnai_fas3_t4t5_batch2/seg0'
output_folder = '/Users/tairan/Downloads/rnai_fas3_t4t5_batch2/outputnew1'
