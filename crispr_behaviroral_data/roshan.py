...
def plot_trajectories(trials_list, title,frames_before_stim=100):
    # Set the frame limits relative to stimulus onset
    frames_before_stim = 200
    frames_after_stim = 500
...
for file_path in glob.glob(f"{folder_path}/*.csv"):
    ...
    for i in range(len(df["direction"]) - 1):
        if df["direction"].iloc[i] == 0 and df["direction"].iloc[i+1] == 1.0:
            ...
            df_trial = align_smoothed_trajectory(df_trial, 200)
    ...
    window_size = 120  # Set window size for rolling straightness
    step_size = 20     # Set step size for rolling straightness
...
def calculate_pre_stim_straightness(x, y, stim_start_idx=100, window_size=45, step_size=1):
    """
    Calculate the straightness of the trajectory before stimulation.
    Returns an array of straightness values.
    """
    straightness_values = []
    for start_idx in range(0, stim_start_idx - window_size + 1, step_size):
        end_idx = start_idx + window_size
        x_window = x[start_idx:end_idx]
        y_window = y[start_idx:end_idx]
...
# Main code to process data and calculate pre-stimulation straightness
folder_path = '/Users/tairan/Downloads/test_roshan'
stim_start_idx=200
...
for file_path in glob.glob(f"{folder_path}/*.csv"):
    ...
    for i in range(len(df["direction"]) - 1):
        if df["direction"].iloc[i] == 0 and df["direction"].iloc[i+1] == -1.0:
            ...
            trials_list.append(trial_data)
            trial_num += 1
    # List to collect pre-stimulation straightness values for each file
    pre_stim_straightness_values = []
    # Filter responsive trials
    speed_threshold = 0.4
    responsive_trials = [trial for trial in trials_list if is_fly_responsive(trial, speed_threshold)]
    
    # Calculate pre-stimulation straightness for responsive trials
    window_size = 50
    step_size = 20
    ...
    for trial in responsive_trials:
        x_aligned = trial['x_aligned']
        y_aligned = trial['y_aligned']
        pre_stim_straightness = calculate_pre_stim_straightness(x_aligned, y_aligned, stim_start_idx, window_size, step_size)
        pre_stim_straightness = pre_stim_straightness[~np.isnan(pre_stim_straightness)]
        pre_stim_straightness_values.extend(pre_stim_straightness)

    # Plot pre-stimulation straightness distribution for the current file
    if pre_stim_straightness_values:
        plt.figure(figsize=(8, 6))
        plt.hist(pre_stim_straightness_values, bins=100, density=True, alpha=0.7, color='blue')
        plt.title(f'Pre-Stimulation Straightness Distribution for {file_name}')
        plt.xlabel('Straightness (D/L)')
        plt.ylabel('Probability Density')
        plt.grid(True)
        plt.show()
    else:
        print(f"No pre-stimulation straightness values to plot for {file_name}.")
