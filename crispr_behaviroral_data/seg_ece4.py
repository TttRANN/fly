...
def process_video_segments(csv_path, video_path, save_dir):
    ...
    # No longer using Var1 as frame number; frame number is the row index
    # Ensure that Var5 is of integer type (assuming it represents state)
    try:
        data['Var5'] = data['Var5'].astype(int)
    except ValueError:
        print(f"Error: 'Var5' column in {csv_path} contains non-integer values.")
        return
    ...
    if start_frame is not None:
        ...
        print(f"Video ended while 'on'. Segment: {start_frame} to {total_frames - 1}")
