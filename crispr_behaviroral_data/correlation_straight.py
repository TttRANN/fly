...
# # Function to process trials and compute average straightness
...
#     # After processing all trials, select responsive trials
...
#         # Calculate average straightness across responsive trials
...
#         return responsive_trials, average_straightness, common_time_array
#     else:
#         print("No responsive trials found.")
#         return [], None, None
...
#     # Process df
#     responsive_trials_df, average_straightness_df, common_time_array_df = process_trials(df)

#     # Process df1
#     responsive_trials_df1, average_straightness_df1, common_time_array_df1 = process_trials(df1)

#     # Plot average straightness for df
#     if average_straightness_df is not None:
#         plt.figure(figsize=(10, 6))
#         plt.plot(common_time_array_df, average_straightness_df, label='Average Straightness df', color='blue')
#         plt.xlabel('Time (frames relative to stimulus onset)')
#         plt.ylabel('Straightness')
#         plt.title(f'Average Straightness for {file_name}')
#         plt.legend()
#         plt.ylim([-1, 1])
#         plt.xlim([common_time_array_df[0], common_time_array_df[-1]])
#         plt.show()
#     else:
#         print(f"No responsive trials found for {file_name}.")

#     # Plot average straightness for df1
#     if average_straightness_df1 is not None:
#         plt.figure(figsize=(10, 6))
#         plt.plot(common_time_array_df1, average_straightness_df1, label='Average Straightness df1', color='green')
#         plt.xlabel('Time (frames relative to stimulus onset)')
#         plt.ylabel('Straightness')
#         plt.title('Average Straightness for df1')
#         plt.legend()
#         plt.ylim([-1, 1])
#         plt.xlim([common_time_array_df1[0], common_time_array_df1[-1]])
#         plt.show()
#     else:
#         print("No responsive trials found for df1.")

#     # Plot both on the same plot for comparison
#     if average_straightness_df is not None and average_straightness_df1 is not None:
#         plt.figure(figsize=(10, 6))
#         plt.plot(common_time_array_df, average_straightness_df, label='df', color='blue')
#         plt.plot(common_time_array_df1, average_straightness_df1, label='df1', color='green')
#         plt.xlabel('Time (frames relative to stimulus onset)')
#         plt.ylabel('Straightness')
#         plt.title('Average Straightness Comparison')
#         plt.legend()
#         plt.ylim([-1, 1])
#         plt.xlim([min(common_time_array_df[0], common_time_array_df1[0]), max(common_time_array_df[-1], common_time_array_df1[-1])])
#         plt.show()

#         # Cross-correlation between average straightness of df and df1
#         # Find overlapping time range
#         overlapping_min_time = max(np.min(common_time_array_df), np.min(common_time_array_df1))
#         overlapping_max_time = min(np.max(common_time_array_df), np.max(common_time_array_df1))
#         common_time_array = np.arange(overlapping_min_time, overlapping_max_time + 1)

#         # Create pandas Series with time as index
#         average_straightness_series_df = pd.Series(average_straightness_df, index=common_time_array_df)
#         average_straightness_series_df1 = pd.Series(average_straightness_df1, index=common_time_array_df1)

#         # Reindex to the common time array
#         average_straightness_series_df = average_straightness_series_df.reindex(common_time_array)
#         average_straightness_series_df1 = average_straightness_series_df1.reindex(common_time_array)

#         # Drop NaN values
#         valid_indices = (~average_straightness_series_df.isna()) & (~average_straightness_series_df1.isna())
#         straightness_df = average_straightness_series_df[valid_indices].values
#         straightness_df1 = average_straightness_series_df1[valid_indices].values

#         # Ensure both arrays are the same length
#         if len(straightness_df) == 0 or len(straightness_df1) == 0:
#             print("No overlapping data to compute cross-correlation.")
#         else:
#             # Compute cross-correlation
#             # Remove mean to center data
#             straightness_df_centered = straightness_df - np.mean(straightness_df)
#             straightness_df1_centered = straightness_df1 - np.mean(straightness_df1)

#             correlation = correlate(straightness_df_centered, straightness_df1_centered, mode='full')
#             lags = np.arange(-len(straightness_df_centered) + 1, len(straightness_df_centered))

#             # Find the lag corresponding to maximum correlation
#             max_corr_index = np.argmax(correlation)
#             delay = lags[max_corr_index]
...
#             plt.title('Cross-correlation between df and df1')
#             plt.axvline(x=delay, color='red', linestyle='--', label=f'Delay = {delay} frames')
#             plt.legend()
#             plt.show()

#             print(f"The delay time between df and df1 is {delay} frames.")

#             # Plot trajectories if desired
#             plot_trajectories(responsive_trials_df, f'Trajectories for {file_name}')
#             plot_trajectories(responsive_trials_df1, 'Trajectories for df1')
#     else:
#         print(f"Not enough data to perform cross-correlation for {file_name}.")
...
# # Define any necessary functions here
# def correct_opencv_theta_assignment(df_trial):
#     # Placeholder function to correct 'theta'
#     return df_trial

# def align_smoothed_trajectory(df_trial, stim_start_idx):
#     # Placeholder function to align the smoothed trajectory
#     df_trial['x_aligned'] = df_trial['pos_x'] - df_trial['pos_x'].iloc[stim_start_idx]
#     df_trial['y_aligned'] = df_trial['pos_y'] - df_trial['pos_y'].iloc[stim_start_idx]
#     return df_trial

# def is_fly_responsive(trial, speed_threshold):
#     # Placeholder function to determine if a fly is responsive
#     return True  # Replace with actual logic

# def plot_trajectories(responsive_trials, title):
#     # Placeholder function to plot trajectories
#     plt.figure()
#     for trial in responsive_trials:
#         plt.plot(trial['x_aligned'], trial['y_aligned'], alpha=0.5)
#     plt.title(f'Trajectories: {title}')
#     plt.xlabel('X Aligned')
#     plt.ylabel('Y Aligned')
#     plt.show()

# Exponential decay function to fit
...
def process_trials(df):
    ...
    # After processing all trials, select responsive trials
    speed_threshold = 0.3  # Define speed threshold
    responsive_trials = [trial for trial in trials_list if is_fly_responsive(trial, speed_threshold)]
...
# Main processing loop
folder_path = '/Users/tairan/Downloads/test_roshan/untitled'  # Update this to your folder path

# Process each CSV file in the folder
for file_path in glob.glob(f"{folder_path}/*.csv"):
    file_name = os.path.basename(file_path)
    file_template_name = '/Users/tairan/Downloads/test_roshan/R42F06-Gal4_control_6_2024117Dark.csv'
    df = pd.read_csv(file_path, names=["pos_x", "pos_y", "ori", "timestamp", "frame_number", "video_frame", "direction"])
    df1 = pd.read_csv(file_template_name, names=["pos_x", "pos_y", "ori", "timestamp", "frame_number", "video_frame", "direction"])
    df = df.apply(pd.to_numeric, errors='coerce')
    df1 = df1.apply(pd.to_numeric, errors='coerce')
    ...
    # Fit exponential decay model to average straightness of df
    if average_straightness_df is not None:
        # We will fit from stimulus onset (time zero) onwards
        fit_indices_df = common_time_array_df >= 0
        t_df = common_time_array_df[fit_indices_df]
        y_df = average_straightness_df[fit_indices_df]

        # Remove NaNs
        valid_indices_df = ~np.isnan(y_df)
        t_df = t_df[valid_indices_df]
        y_df = y_df[valid_indices_df]

        # Fit the exponential decay model
        try:
            popt_df, pcov_df = curve_fit(exponential_decay, t_df, y_df)
            A_df, tau_df, C_df = popt_df

            # Plot the average straightness and the fitted model
            plt.figure(figsize=(10, 6))
            plt.plot(common_time_array_df, average_straightness_df, label='Average Straightness df', color='blue')
            plt.plot(t_df, exponential_decay(t_df, *popt_df), 'r--', label=f'Fitted Exp Decay (tau={tau_df:.2f})')
            plt.xlabel('Time (frames relative to stimulus onset)')
            plt.ylabel('Straightness')
            plt.title(f'Average Straightness and Exponential Decay Fit for {file_name}')
            plt.legend()
            plt.ylim([-1, 1])
            plt.xlim([common_time_array_df[0], common_time_array_df[-1]])
            plt.show()
        except RuntimeError:
            print("Exponential fit did not converge for df.")
            plt.figure(figsize=(10, 6))
            plt.plot(common_time_array_df, average_straightness_df, label='Average Straightness df', color='blue')
            plt.xlabel('Time (frames relative to stimulus onset)')
            plt.ylabel('Straightness')
    ...
    # Fit exponential decay model to average straightness of df1
    if average_straightness_df1 is not None:
        # We will fit from stimulus onset (time zero) onwards
        fit_indices_df1 = common_time_array_df1 >= 0
        t_df1 = common_time_array_df1[fit_indices_df1]
        y_df1 = average_straightness_df1[fit_indices_df1]

        # Remove NaNs
        valid_indices_df1 = ~np.isnan(y_df1)
        t_df1 = t_df1[valid_indices_df1]
        y_df1 = y_df1[valid_indices_df1]

        # Fit the exponential decay model
        try:
            popt_df1, pcov_df1 = curve_fit(exponential_decay, t_df1, y_df1, p0=(1, 10, 0))
            A_df1, tau_df1, C_df1 = popt_df1

            # Plot the average straightness and the fitted model
            plt.figure(figsize=(10, 6))
            plt.plot(common_time_array_df1, average_straightness_df1, label='Average Straightness df1', color='green')
            plt.plot(t_df1, exponential_decay(t_df1, *popt_df1), 'r--', label=f'Fitted Exp Decay (tau={tau_df1:.2f})')
            plt.xlabel('Time (frames relative to stimulus onset)')
            plt.ylabel('Straightness')
            plt.title('Average Straightness and Exponential Decay Fit for df1')
            plt.legend()
            plt.ylim([-1, 1])
            plt.xlim([common_time_array_df1[0], common_time_array_df1[-1]])
            plt.show()
        except RuntimeError:
            print("Exponential fit did not converge for df1.")
            plt.figure(figsize=(10, 6))
            plt.plot(common_time_array_df1, average_straightness_df1, label='Average Straightness df1', color='green')
            plt.xlabel('Time (frames relative to stimulus onset)')
            plt.ylabel('Straightness')
    ...
    # Plot both averaged straightness curves on the same plot for comparison
    if average_straightness_df is not None and average_straightness_df1 is not None:
        plt.figure(figsize=(10, 6))
        plt.plot(common_time_array_df, average_straightness_df, label='df', color='blue')
        plt.plot(common_time_array_df1, average_straightness_df1, label='df1', color='green')
        ...
        # Plot the exponential decay fits together
        plt.figure(figsize=(10, 6))
        plt.plot(common_time_array_df, average_straightness_df, label='Average Straightness df', color='blue')
        plt.plot(t_df, exponential_decay(t_df, *popt_df), 'b--', label=f'df Fit (tau={tau_df:.2f})')
        plt.plot(common_time_array_df1, average_straightness_df1, label='Average Straightness df1', color='green')
        plt.plot(t_df1, exponential_decay(t_df1, *popt_df1), 'g--', label=f'df1 Fit (tau={tau_df1:.2f})')
        plt.xlabel('Time (frames relative to stimulus onset)')
        plt.ylabel('Straightness')
        plt.title('Average Straightness and Exponential Decay Fits')
        plt.legend()
        plt.ylim([-1, 1])
        plt.xlim([0, max(common_time_array_df[-1], common_time_array_df1[-1])])
        plt.show()

        # Optionally, print decay factors
        print(f"Decay factor (tau) for df: {tau_df:.2f} frames")
        print(f"Decay factor (tau) for df1: {tau_df1:.2f} frames")

    else:
        print(f"Not enough data to perform comparison for {file_name}.")
